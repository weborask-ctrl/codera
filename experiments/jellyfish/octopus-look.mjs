...
if(study){
 try {
  const {createOctopus}=await import('./octopus-blender.mjs');
...
let characterSwimming=false,characterEffort=0;
const motionButton=document.querySelector('#character-motion');
if(motionButton)motionButton.onclick=()=>{characterSwimming=!characterSwimming;motionButton.textContent=characterSwimming?'Pokoj':'Plávanie';dirty=true;wake();};
...
function draw(){
 ...
 if(jelly?.kind==='blender'&&!journey){characterEffort+=(Number(characterSwimming)-characterEffort)*.1;jelly.setMotion(characterEffort,elapsed*.7+.95,[],{progress:0,contact:false,reduced:reduced.matches});}
