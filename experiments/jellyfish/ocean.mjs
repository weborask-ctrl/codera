...
const study=document.body.hasAttribute('data-jellyfish');
...
const uniforms={cameraOffset:{value:new THREE.Vector3()},resolution:{value:new THREE.Vector2()},time:{value:0},depth:{value:3},travel:{value:0},pointer:{value:new THREE.Vector2()},waveMap:{value:null},useWaveMap:{value:false},renderMode:{value:0},volumeMap:{value:null}};
...
if(study){
 const {createGelJellyfish}=await import('./gel-jellyfish.mjs');
 jelly=createGelJellyfish(uniforms.time,uniforms.waveMap);
 jelly.group.position.set(1.7,-1.35,-6.5);
 jelly.group.rotation.set(.06,.22,-.15);
 jelly.group.scale.setScalar(.86);
 jellyScene.add(jelly.group);
 uniforms.cameraOffset.value.y=-1;
 ...
 document.querySelectorAll('[data-view]').forEach(button=>button.onclick=()=>{
  ...
  uniforms.cameraOffset.value.set(detail?jelly.group.position.x-.3:0,detail?.05:-1,detail?-3.5:0);
