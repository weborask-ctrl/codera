...
import { oceanFragment as fragment, waveMapFragment } from './ocean-shaders.mjs';
...
const uniforms={resolution:{value:new THREE.Vector2()},time:{value:0},depth:{value:3},travel:{value:0},pointer:{value:new THREE.Vector2()},waveMap:{value:null},useWaveMap:{value:false},renderMode:{value:0},volumeMap:{value:null}};
...
if(renderer.extensions.has('EXT_color_buffer_float')){
 waveTarget=new THREE.WebGLRenderTarget(1024,1024,{type:THREE.HalfFloatType,depthBuffer:false,stencilBuffer:false,wrapS:THREE.RepeatWrapping,wrapT:THREE.RepeatWrapping,minFilter:THREE.LinearMipmapLinearFilter,magFilter:THREE.LinearFilter,generateMipmaps:true});
...
function resize(){const budget=Number(document.querySelector('#quality').value);const ratio=Math.min(devicePixelRatio,2,Math.sqrt(budget/(innerWidth*innerHeight)));renderer.setPixelRatio(ratio);renderer.setSize(innerWidth,innerHeight,false);if(volumeTarget)volumeTarget.setSize(Math.max(1,Math.ceil(innerWidth*ratio*.5)),Math.max(1,Math.ceil(innerHeight*ratio*.5)));uniforms.resolution.value.set(innerWidth,innerHeight);particleCamera.aspect=innerWidth/innerHeight;particleCamera.updateProjectionMatrix();particles.uniforms.pixels.value=ratio;dirty=true;wake();}
function draw(){
 uniforms.time.value=elapsed;const t=uniforms.travel.value;
 particleCamera.position.set(t*1.4,-uniforms.depth.value+Math.sin(elapsed*.23)*.055,t*5);
 particleCamera.lookAt(particleCamera.position.clone().add(new THREE.Vector3(uniforms.pointer.value.x*.12,.35+uniforms.pointer.value.y*.08,-1)));
...
addEventListener('pagehide',event=>{
 ...
 if(!event.persisted){waveTarget?.dispose();volumeTarget?.dispose();volumeScene.traverse(object=>object.material?.dispose());waveScene.traverse(object=>object.material?.dispose());material.dispose();plane.geometry.dispose();particles.dispose();geometry.dispose();renderer.dispose();}
});
