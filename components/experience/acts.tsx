 ...
 *
 * Natural document flow over the fixed world canvas. Sticky regions do
 * the choreography (no ScrollTrigger pins): /02's fold is a clip-path
 * driven by the region's own progress, /03 is a native sticky stack.
 * A single rAF writer maps `stage` to CSS variables — paint-only
 * properties, written raw (no smoothing between input and DOM).
 *
 * Text choreography: entrances via IntersectionObserver (enter → settle
 * at full opacity); copy never lives below full legibility during its
 * hold (Step 5 §2.3).
 */
...
function useStage(probe: boolean) {
  ...
  useEffect(() => {
    ...
    const write = () => {
      ...
      /* /02 fold: completes at 70% of the region, then HOLDS readable */
      const fold = Math.min(1, stage.p.premena / 0.7)
      root.style.setProperty("--fold", fold.toFixed(4))
      /* /03 stack: receding transforms for covered stages */
    ...
    /* entrances: settle-once, never scrubbed */
...
function ActHero() {
  return (
    <section data-zone="hero" className="relative flex h-svh flex-col text-[#f4f1ea]">
      <div data-enter className="enter relative z-10 mt-auto mb-[9svh] px-[clamp(1.25rem,4vw,3.5rem)]">
        ...
        <h1
          ...
          style={{ fontSize: "clamp(2.4rem,6.6vw,6.9rem)", lineHeight: 0.98, letterSpacing: "-0.035em" }}
        >
...
function ActPremena() {
  return (
    <section data-zone="premena" data-zone-sticky id="premena" className="relative h-[180svh]">
      <div className="sticky top-0 flex h-svh flex-col text-[#16171b]">
        ...
        <div className="relative mx-[clamp(1.25rem,4vw,3.5rem)] mb-[6svh] flex-1 overflow-hidden rounded-[10px] shadow-[0_30px_80px_rgba(20,20,25,0.18)]">
          <div className="absolute inset-0">
            <BilancAfter />
          </div>
          {/* the fold: before-surface swept away by --fold (0 → 1) */}
          <div
...
const STAGES = [
  {
    id: "konstrukt",
    meta: "01 / KONŠTRUKT — STAVEBNÍCTVO",
    tone: "#e3e2dd",
    ink: "#1d1e20",
  },
  {
    id: "vitalis",
    meta: "02 / VITALIS — ZDRAVOTNÍCTVO",
    tone: "#faf7f1",
    ink: "#22312e",
  },
  {
    id: "forma",
    meta: "03 / FORMA — INTERIÉROVÝ ATELIÉR",
    tone: "#f2ecdf",
    ink: "#211d17",
  },
] as const
      ...
      <div className="flex flex-1">
        <div className="flex flex-1 flex-col justify-center px-7">
          ...
          <p
            ...
            style={{ fontSize: "clamp(1.9rem,4vw,4.2rem)", lineHeight: 0.92, fontStretch: "125%" }}
          >
            Postavené
            <br />
            presne.
    ...
    <div className="flex h-full flex-col" style={{ color: "#22312e" }}>
      ...
      <div className="flex flex-1 items-center gap-8 px-7">
        <div className="max-w-[30em] flex-1">
          ...
          <p
            className="font-semibold"
            style={{ fontSize: "clamp(1.8rem,3.4vw,3.2rem)", lineHeight: 1.02, letterSpacing: "-0.028em" }}
          >
            Termín do 48 hodín.
            <br />
            Bez čakania v rade.
...
function StageForma() {
  return (
    <div className="flex h-full flex-col" style={{ color: "#211d17" }}>
      ...
      <div className="flex flex-1 px-7 pt-[4svh]">
        <div className="flex-1">
          <p
            className="font-serif"
            style={{ fontSize: "clamp(2rem,4.2vw,4.2rem)", lineHeight: 1.0 }}
          >
            Interiéry, ktoré
            <br />
...
function ActWork() {
  const stages = [<StageKonstrukt key="k" />, <StageVitalis key="v" />, <StageForma key="f" />]
  return (
    <section data-zone="work" data-zone-sticky id="praca" className="relative h-[300svh]">
      {STAGES.map((s, i) => (
        <div key={s.id} className="sticky top-0 h-svh" style={{ zIndex: i + 1 }}>
          <div
            className="flex h-full flex-col px-[clamp(1.25rem,4vw,3.5rem)] py-[4svh]"
            style={
              i < 2
                ? {
...
function ActOffer() {
  return (
    <section data-zone="offer" id="sluzby" className="relative flex min-h-svh flex-col text-[#1b1c20]">
      <header className="flex items-baseline justify-between px-[clamp(1.25rem,4vw,3.5rem)] pt-8">
      ...
      <div className="mx-[clamp(1.25rem,4vw,3.5rem)] mt-[4svh] mb-[6svh] flex flex-1 flex-col justify-center">
        {[
          ...
          <div
            key={n}
            data-enter
            className="enter grid grid-cols-[3rem_1fr] items-baseline gap-x-6 border-t border-black/12 py-[3.2svh] md:grid-cols-[4rem_minmax(12rem,22rem)_1fr]"
          >
            <span className="font-mono text-[0.7rem] text-black/40">{n}</span>
            <span className="font-semibold" style={{ fontSize: "clamp(1.5rem,3vw,2.8rem)", letterSpacing: "-0.02em" }}>
              {t}
            </span>
...
function ActResolution() {
  return (
    <section data-zone="resolution" id="kontakt" className="relative flex min-h-svh flex-col text-[#1b1c20]">
      <div data-enter className="enter flex flex-1 flex-col items-center justify-center px-[clamp(1.25rem,4vw,3.5rem)] pt-[30svh] text-center">
        <h2
          className="font-semibold"
...
export function ExperienceActs({ probe = false }: { probe?: boolean }) {
  const probeRef = useStage(probe)
  return (
    <main id="experience-root" className="relative z-10">
      <ActHero />
      <div data-zone="pass" aria-hidden="true" className="h-[60svh]" />
      <ActPremena />
      <ActWork />
      <ActOffer />
      <ActResolution />
