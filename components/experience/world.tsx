...
function desiredPose(): Pose {
  ...
  switch (act) {
    ...
    case "offer":
      /* /03 (Iterácia 0.5b): the stones themselves ARE the letter — a
         symmetric stone C under the act heading, full presence, no
         hand-over (the ink-letter morph was cut: one material language) */
      return {
        cam: [0, 0, 3.4],
        target: [0, 0, 0],
        molten: 0,
    ...
    if (shardsGroup) {
      ...
      for (const obj of shardsGroup.children) {
        ...
        if (!seed.baseScale) {
          ...
          /* the offer letter must read SYMMETRIC (Ondrej): every stone
             grows/shrinks toward one shared size inside the C */
          seed.norm =
        ...
        if (act === "hero") {
          ...
          /* the stone letter (Iterácia 0.5b): even spacing, tight jitter —
             the C reads symmetric, no bulging side */
          const t = i / (SHARD_COUNT - 1)
          const a = C_GAP + t * (Math.PI * 2 - C_GAP * 2)
          shardTarget.set(
