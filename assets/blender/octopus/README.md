...
- Older renders remain as iteration evidence. This static master has no authored swimming or crawling clips. The separate animated files `codera-octopus-swim.blend` and `.glb` contain [Motion III](MOTION_III.md): repaired arm weights, swimming, steering and small rise/descent. [SWIM_STUDY.md](SWIM_STUDY.md) retains earlier iterations.
