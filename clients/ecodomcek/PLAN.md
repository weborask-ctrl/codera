...
| **2 · Art direction + boards** — *draft 1 done 2026-09-03 (`ART_DIRECTION.md`, `boards/out/`), on Ondrej's instruction ahead of phase 1; gate open* | `ART_DIRECTION.md` citing record IDs; static boards: desktop Príjazd / Obývačka / Tech X-ray / Dollhouse / Kontakt; mobile Príjazd / a room / Kontakt; type candidates tested with the longest Slovak strings and diacritics | Boards + AD doc | Static-frame test; side-by-side with the references; Ondrej's pass |
| **3 · Model production** | Blender: house from plans, maquette materials, four light bakes, exploded wall asset, GLB + KTX2 export, per-room stills, dollhouse plate | `public/house/*.glb`, stills | Budgets met (≤ 4 MB GLB, ≤ 6 MB textures, ≤ 150 k tris); stills pass the frame test |
