...
(async () => {
  ...
  const state = () => page.evaluate(() => ({ hash: location.hash, page: document.body.dataset.page, slabs: document.querySelectorAll('.slab').length, cards: document.querySelectorAll('.job').length, curtain: getComputedStyle(document.getElementById('curtain')).display, built: !!document.querySelector('.poster.built') }));
  ...
  await page.click('.job'); await page.waitForTimeout(2000); console.log('detail', JSON.stringify(await state()), await page.evaluate(() => document.querySelector('h1').textContent.trim().slice(0,30)));
