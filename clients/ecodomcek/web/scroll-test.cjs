...
async function run(w, h, tag) {
  ...
  const res = [];
  for (const t of [0, .26, .51, .76, 1]) {
    await page.evaluate((t) => { const a = document.getElementById('hero'); scrollTo(0, (a.offsetHeight - innerHeight) * t); }, t);
    await page.waitForTimeout(1000);
    res.push(await page.evaluate(() => { const b = document.getElementById('house').getBoundingClientRect(); const r = document.querySelector('.lyr[data-l=roof]').getBoundingClientRect(); const s = document.querySelector('#hero .sticky').getBoundingClientRect(); return [Math.round(scrollY), Math.round(s.top), Math.round(b.top), Math.round(b.height), Math.round(r.top)]; }));
