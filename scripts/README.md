...
- `npm run prototype:jellyfish:browser -- [url] [output-directory] [--functional]`: sequential Chromium regression and performance checks for the standalone preview (default `http://127.0.0.1:4317`, output `test-results/jellyfish`). Requires installed Playwright Chromium, the preview running, and enough free RAM for one browser scene. Covers native forward/reverse scrolling, portfolio/static ticker sleep, context loss/restart, deep links, reduced motion, no JavaScript, unavailable libraries/health API, memory safeguards, and six viewport sizes. Saves compact screenshots and `report.json`. Default also measures three 6-second native-scroll legs per desktop/mobile/CPU-throttled profile, using actual submitted WebGL frames and recording the real renderer; close other animated browser tabs first. `--functional` skips performance; `JELLYFISH_TEST` may narrow regression names during debugging (the report records the filter, and zero matching tests fails). Emulation is not real-phone validation; synthetic lifecycle events verify handlers, not browser bfcache eligibility.
