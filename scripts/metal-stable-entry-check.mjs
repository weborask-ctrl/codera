...
try {
  ...
  await page.locator('#entry-loader button').click();
  assert.equal(await page.evaluate(() => window.__coderaMotion.active), false);
  console.log('PASS static entry while complete film prepares', Date.now() - start);
  await page.screenshot({ path: 'test-results/entry/static-preparation.png' });
  ...
  assert.equal(await page.evaluate(() => window.__coderaMotion.fullyBuffered), true);
  assert.equal(await page.evaluate(() => window.__coderaMotion.delivery), 'complete-file-blob');
  ...
  console.log(`PASS explicit motion only after complete preparation (${coldMs} ms at 10 Mbit/s), native-scroll lock during loader, full resolution, reverse/endpoints without network`);
  await cdp.send('Network.emulateNetworkConditions', { offline: false, latency: 60, downloadThroughput: 1250000, uploadThroughput: 1250000 });
  ...
  await automatic.route('**/*.mp4', () => {});
  const automaticStart = Date.now();
  await automatic.goto(url, { waitUntil: 'domcontentloaded' });
  await automatic.waitForFunction(() => !document.documentElement.classList.contains('codera-loading'), null, { timeout: 6500 });
  assert.equal(await automatic.evaluate(() => window.__coderaMotion.active), false);
  assert.equal(await automatic.locator('main').evaluate(e => e.inert), false);
  assert.equal(await automatic.locator('.journey').evaluate(e => e.offsetHeight), 900);
  console.log('PASS bounded automatic static entry without incomplete motion', Date.now() - automaticStart);
